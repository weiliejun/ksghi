...
public class HttpRequestDeviceUtils {
     ...
     * 手机浏览器的User-Agent里的关键词
     */
    private static String[] iosUserAgents = new String[]{"iPhone", // iPhone是否也转wap？不管它，先区分出来再说。Mozilla/5.0
            // (iPhone;
            // U; CPU
            // iPhone OS 4_1 like Mac OS X; zh-cn) AppleWebKit/532.9
            // (KHTML like Gecko) Mobile/8B117
    ...
    public static boolean isIosDevice(HttpServletRequest request) {
        ...
        for (int i = 0; !mobileFlag && userAgent != null && !userAgent.trim().equals("") && i < iosUserAgents.length; i++) {
