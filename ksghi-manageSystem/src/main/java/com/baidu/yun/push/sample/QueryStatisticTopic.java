...
public class QueryStatisticTopic {
    public static void main(String[] args) throws PushClientException, PushServerException {
        ...
        try {
            ...
            QueryStatisticTopicRequest request = new QueryStatisticTopicRequest().addTopicId("BaiduPush").addDeviceType(3);
            // 5. http request
            ...
            if (null != response) {
                ...
                for (int i = 0; i < topicStats.size(); i++) {
                    ...
                    strBuilder.append("" + keyValue.getKey() + ":{" + "ackNum: " + keyValue.getValue().getAckNum() + "}");
                }
                strBuilder.append("}");
