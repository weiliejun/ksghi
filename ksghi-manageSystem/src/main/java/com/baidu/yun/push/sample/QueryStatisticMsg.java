...
public class QueryStatisticMsg {
    public static void main(String[] args) throws PushClientException, PushServerException {
        ...
        try {
            ...
            QueryStatisticMsgRequest request = new QueryStatisticMsgRequest().addQueryType(0).addDeviceType(3);
            // 5. http request
            ...
            if (null != response) {
                ...
                for (int i = 0; i < msgStats.size(); i++) {
                    ...
                    for (int j = 0; j < result.size(); j++) {
                        ...
                        strBuilder.append("" + keyValue.getKey() + ":{ " + "pushNum=" + keyValue.getValue().getPushNum() + ",ackNum=" + keyValue.getValue().getAckNum() + ",delNum=" + keyValue.getValue().getDelNum() + ",clickNum=" + keyValue.getValue().getClickNum() + "}");
                    }
                    strBuilder.append("}");
                }
