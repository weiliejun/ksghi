...
public class IOSPushNotificationToTag {
    ...
    public static void main(String[] args) throws PushClientException, PushServerException {
        ...
        try {
            ...
            PushMsgToTagRequest request = new PushMsgToTagRequest().addTagName("xxxxx") // 设置tag组
                    .addMsgExpires(new Integer(3600)).addMessageType(1) // 1：通知,0:透传消息.默认为0
                    // 注：IOS只有通知.
                    // .addSendTime(System.currentTimeMillis() / 1000 + 86400)
                    // //设置定时推送时间
                    .addMessage(notification.toString()).addDeployStatus(2)// IOS,DeployStatus
                    // =>
                    // 1:
                    // Developer,
                    // 2:
