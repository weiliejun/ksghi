...
public class QueryDeviceNumInTag {
    public static void main(String[] args) throws PushClientException, PushServerException {
        ...
        try {
            ...
            QueryDeviceNumInTagRequest request = new QueryDeviceNumInTagRequest().addTagName("xxxxx").addDeviceType(3);
            // 5. http request
            QueryDeviceNumInTagResponse response = pushClient.queryDeviceNumInTag(request);
            if (null != response) {
                System.out.println(String.format("deviceNum: %d", response.getDeviceNum()));
            }
