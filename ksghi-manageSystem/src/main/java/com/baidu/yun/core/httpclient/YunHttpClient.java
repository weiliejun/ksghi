...
public class YunHttpClient implements YunHttpObservable {
    ...
    public HttpRestResponse doExecutePostRequestResponse(String surl, Map<String, String> params) throws YunHttpClientException {
        ...
        for (int i = 0; i < maxRetryTimes; i++) {
            ...
            try {
                ...
                if (i + 1 >= maxRetryTimes) {
                    ...
                    // @notify log event
                    notifyAndCallback(new YunHttpEvent(YunHttpEvent.TCP_CONNECT_FAIL, surl, paramsContent, httpStatusCode, "Network connect fail, domain[" + BaiduPushConstants.CHANNEL_REST_URL + "] exists?"));

                    throw new YunHttpClientException("Network connect fail, domain[" + BaiduPushConstants.CHANNEL_REST_URL + "] exists?");
            ...
            try {
                ...
                if (httpStatusCode == 200) {
                    ...
                    // @notify log event
                    notifyAndCallback(new YunHttpEvent(YunHttpEvent.NETWORK_TALK_SUCCESS, surl, paramsContent, httpStatusCode, responseContent));
                    ...
                    if (httpConfigure != null && httpConfigure.isRelocationable()) {
                        ...
                        if (location != null) {
                            ...
                            // @notify log event
                            notifyAndCallback(new YunHttpEvent(YunHttpEvent.NETWORK_NORMAL, surl, paramsContent, httpStatusCode, "redirect to " + locationUrl));

                        } else {
                            // @notify log event, notify header location is
                            // null, so give up to
                            notifyAndCallback(new YunHttpEvent(YunHttpEvent.NETWORK_NORMAL, surl, paramsContent, httpStatusCode, "header location is null, so give up to redirect"));
                        }
                    } else {
                        // @notify log event, redirect location event....
                        notifyAndCallback(new YunHttpEvent(YunHttpEvent.NETWORK_NORMAL, surl, paramsContent, httpStatusCode, "web server suggest redirect, but client don't support it"));
                    }
                    continue;
                } else {
                    ...
                    // @notify log event
                    notifyAndCallback(new YunHttpEvent(YunHttpEvent.NETWORK_NORMAL, surl, paramsContent, httpStatusCode, responseContent));
                ...
                // @notify log event
                notifyAndCallback(new YunHttpEvent(YunHttpEvent.NETWORK_IO_EXCEPTION, surl, paramsContent, httpStatusCode, e.getMessage()));
                continue;
        ...
        // @notify log event
        notifyAndCallback(new YunHttpEvent(YunHttpEvent.NETWORK_TRY_TIMES_EXCEPTION, surl, paramsContent, httpStatusCode, responseContent));
