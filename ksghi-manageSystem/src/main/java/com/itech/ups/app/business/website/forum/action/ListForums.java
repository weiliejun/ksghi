 ...
 */
@Controller
public class ListForums extends AbstractActionParent {

    @Autowired
    private ForumService forumService;

    @RequestMapping("/business/website/forum/list")
    public String list(HttpServletRequest request, Model model) {
        ...
        if (new Integer(params.get("size").toString()).intValue() >= 0) {
            Collection<?> items = TableModelUtils.getItems("grid", "restore", request, new PageItems() {
                public Collection<?> getItems(Limit limit) {
                    ...
                    return forumService.findPageForums(params, rowStart, rowEnd);
                }

                public int getTotalRows(Limit limit) {
                    return (int) forumService.findForumsTotalCount(params);
                }
            });
        ...
        // ////
        return "/business/website/forum/list";
    }
}
