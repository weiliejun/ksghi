 ...
 */
@Controller
public class ListRoles extends AbstractActionParent {
    @Autowired
    private AuthorityService service;

    @RequestMapping("/system/authority/role/list")
    public String list(HttpServletRequest request, Model model) {
        ...
        if (new Integer(params.get("size").toString()).intValue() >= 0) {
            Collection<?> items = TableModelUtils.getItems("grid", "restore", request, new PageItems() {
                public Collection<?> getItems(Limit limit) {
                    ...
                    return service.findRoles(params, rowStart, rowEnd);
                }

                public int getTotalRows(Limit limit) {
                    return (int) service.findRolesTotalCount(params);
                }
            });
        ...
        return "system/authority/role/list";
    }
}
