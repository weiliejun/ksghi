...
public class XqyInfoImportExcel {
    // 获取map字段映射集合
    ...
    public static void main(String[] args) throws Exception {
        XqyInfoImportExcel rf = new XqyInfoImportExcel();
        // s = rf.readEXCEL("d:/02.xls");
        XqyInfoImportData data = new XqyInfoImportData();
        ...
        List<XqyInfoImportData> dt = rf.readEXCEL2003(is, data.getClass());
        // List<XqyInfoImportData> dt2 =
        // rf.readEXCEL2007("D:/document/产品动态数据_2015-04-03 -
        // 副本.xlsx",data2.getClass());
        for (int i = 0; i < dt.size(); i++) {
            XqyInfoImportData d = (XqyInfoImportData) dt.get(i);
    ...
    // 重新创建实体,防止XqyInfoImportData存的对象永远是一个
    public XqyInfoImportData getnewData(XqyInfoImportData data) throws Exception {
        XqyInfoImportData dt = data.getClass().newInstance();
        // XqyInfoImportData dt = new XqyInfoImportData();
    ...
    // 读取xls文件
    @SuppressWarnings("deprecation")
    public List<XqyInfoImportData> readEXCEL2003(InputStream is, Class cla) throws Exception {

        Map<Object, String> mp = getMap(cla);
        List<XqyInfoImportData> returndata = new ArrayList<XqyInfoImportData>();// 返回结果
        StringBuilder content = new StringBuilder();
        HSSFWorkbook workbook = new HSSFWorkbook(is);// 创建对Excel工作簿文件的引用
        ...
        if (null != workbook.getSheetAt(numSheets)) {
            ...
            for (int rowNumOfSheet = 1; rowNumOfSheet <= aSheet.getLastRowNum(); rowNumOfSheet++) {
                if (null != aSheet.getRow(rowNumOfSheet)) {
                    ...
                    // 第一行标题行不存
                    if (rowNumOfSheet > 1) {
                        returndata.add(getnewData((XqyInfoImportData) data));
                    }
                }
                // }
            }
    ...
    // 读取xlsx文件
    public List<XqyInfoImportData> readEXCEL2007(InputStream is, Class cla) throws Exception {
        // 获取实际对象
        try {
            Map<Object, String> mp = getMap(cla);
            List<XqyInfoImportData> returndata = new ArrayList<XqyInfoImportData>();// 返回结果
                        ...
                        for (short cellNumOfRow = 1; cellNumOfRow <= aRow.getLastCellNum(); cellNumOfRow++) {
                            if (null != aRow.getCell(cellNumOfRow)&&StringHelper.isNotBlank(aRow.getCell(cellNumOfRow).toString())) {
                                ...
                                if (rowNumOfSheet == 1 && !"".equals(value)) {
                                    ...
                                    Field fd = objmp.get(cellNumOfRow);System.out.println(value);
                                    fd.setAccessible(true);// 关键。。。可访问私有变量。
                                    if (value != null) {
                                        Class type = fd.getType();
                                        if (type == String.class) {
