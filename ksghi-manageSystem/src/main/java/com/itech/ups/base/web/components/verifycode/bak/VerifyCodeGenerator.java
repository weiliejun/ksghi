package com.itech.ups.base.web.components.verifycode.bak;
...
/**
 * @author chengang 验证码生成器，使用此类需要将表单里的验证码输入框的name属性设为"verifycode"
...
public class VerifyCodeGenerator {
    ...
    private final String ATTRIBUTE_NAME = "WEBSITE_VERIFYCODE";
    // 图片的宽度
    ...
    public boolean checkVerifyCode(HttpServletRequest request) {
        if (((String) request.getParameter(ATTRIBUTE_NAME)).equalsIgnoreCase((String) request.getSession().getAttribute(ATTRIBUTE_NAME))) {
    ...
    public String getSessionVerifyCode(HttpServletRequest request) {
        return (String) request.getSession().getAttribute(ATTRIBUTE_NAME);
    }
}
