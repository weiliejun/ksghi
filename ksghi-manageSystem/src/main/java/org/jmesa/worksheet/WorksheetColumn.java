...
/**
 * <p>
 * A WorksheetColumn represents the edited HtmlColumn. The originalValue is the
 * value that was originally pulled from the bean (ie, pulled from database).
 * This allows a table to revert back to the original value if needed. The
 * changedValue is the value that was modified by the user.
 * </p>
 *
 * <p>
 * You are also able to register an error with the column. I debated being able
 * to put more than one error, but the GUI would have a hard time dealing with
 * multiple errors anyway so I kept this down to one error.
 * </p>
 *
...
public class WorksheetColumn implements Serializable {
    ...
    /**
     * Equality is based on the property. In other words no two Column Objects
