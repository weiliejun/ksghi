...
/**
 * <p>
 * A WorksheetRow contains WorksheetColumns. A row also has a given status. It
 * can be a newly added row, a modified row, or a deleted row.
 * </p>
 *
 * <p>
 * If a row is modified and then later deleted (before being saved) then it will
 * be deleted from the Worksheet. There is no reason for anyone to be aware that
 * a row was almost created. That should make sense. On the other hand if you
 * delete a row that existed previously (ie, the row exists in the database)
 * then you have to flag the row as being deleted so that you would know to
...
public class WorksheetRow implements Serializable {
    ...
    /**
     * @return Map in which the map keys are the item properties and the map
     * values are the item values.
     */
    ...
    /**
     * See if any of the columns contain errors. If you only want to see if
     * there are errors on the row itself call the Row.hasError() method.
     *
