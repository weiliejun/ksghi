...
public class CheckboxWorksheetEditor extends AbstractWorksheetEditor {
    ...
    /**
     * Interpret the value of the column to be checked or unchecked. The
     * acceptable values for checked is 'y', 'yes', 'true', and '1'. The
     * acceptable values for unchecked is 'n', 'no', 'false' and '0'.
     *
    ...
    public String getValueForWorksheet(Object item, String property, int rowcount) {
        ...
        if (valueToConvert.equalsIgnoreCase("y") || valueToConvert.equalsIgnoreCase("yes") || valueToConvert.equalsIgnoreCase("true") || valueToConvert.equals("1")) {
            return CHECKED;
        }

        if (valueToConvert.equalsIgnoreCase("n") || valueToConvert.equalsIgnoreCase("no") || valueToConvert.equalsIgnoreCase("false") || valueToConvert.equals("0")) {
            return UNCHECKED;
        }
    ...
    protected String getWsColumn(Object value, Object item) {
        ...
        if (isRowRemoved(getCoreContext().getWorksheet(), getColumn().getRow(), item)) {
            ...
            html.onclick(getUniquePropertyJavaScript(item) + "jQuery.jmesa.submitWsCheckboxColumn(this,'" + limit.getId() + "'," + UNIQUE_PROPERTY + ",'" + getColumn().getProperty() + "')");
        }
