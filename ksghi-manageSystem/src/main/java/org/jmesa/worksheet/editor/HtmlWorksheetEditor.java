...
/**
 * Deals with CellEditors when the the table is being used as an editable
 * worksheet. Wraps an existing CellEditor. If the cell was edited then will
 * return the edited value, otherwise return the value of the regular
 * CellEditor.
 *
...
public class HtmlWorksheetEditor extends AbstractWorksheetEditor {
    /**
     * Return either the edited worksheet value, or the value of the underlying
     * CellEditor.
     */
    ...
    protected String getWsColumn(WorksheetColumn worksheetColumn, Object value, Object item) {
        ...
        html.onclick(getUniquePropertyJavaScript(item) + "$.jmesa.createWsColumn(this, '" + limit.getId() + "'," + UNIQUE_PROPERTY + ",'" + getColumn().getProperty() + "')");
        html.close();
        html.append(value);
        html.divEnd();
