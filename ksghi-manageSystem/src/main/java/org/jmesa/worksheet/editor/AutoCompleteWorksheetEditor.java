...
public abstract class AutoCompleteWorksheetEditor extends AbstractWorksheetEditor {
    ...
    protected String getWsColumn(WorksheetColumn worksheetColumn, Object value, Object item, String property) {
        ...
        html.onclick(getUniquePropertyJavaScript(item) + "$.jmesa.createWsAutoCompleteColumn(this, '" + limit.getId() + "'," + UNIQUE_PROPERTY + ",'" + getColumn().getProperty() + "','" + urlValue + "','" + getOptions(item, property) + "')");
        html.close();
        html.append(value);
        html.divEnd();
