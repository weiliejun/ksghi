...
/**
 * Will store the Worksheet object in the users session by the table id.
 * However, once the servlet is set up the developer will not ever have to deal
 * with the fact that the Worksheet object is in session. The ajax calls will
 * abstract that out from the html table side. Then the TableFacade will
 * abstract out the retrieve of the Worksheet in the controller.
 *
...
public class WorksheetUpdater {
    ...
    /**
     * Validate that the columns original value is not the same as the changed
     * value. If it is then remove the column from the row.
     */
