...
public class HtmlHeaderEditor extends AbstractHeaderEditor {
     ...
     * @return The JavaScript to get the next Order when invoking the onlick
     * command.
     */
    protected String onclick(Order currentOrder, HtmlColumn column, Limit limit) {
        ...
        if (currentOrder == Order.NONE) {
            ...
            html.onclick("jQuery.jmesa.addSortToLimit('" + limit.getId() + "','" + position + "','" + column.getProperty() + "','" + currentOrder.toParam() + "');" + getOnInvokeActionJavaScript(limit));
        }
